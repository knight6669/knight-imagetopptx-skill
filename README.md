...
`ppt_text_fit.py` 是溢出预检工具，不是把文字一律缩小的指令。复刻时应先按边框、图标和真实内边距确定完整的可用文本槽，再测量字号。

- 为标题、长单行标签、双行栏头和中英混排短语记录参考字号、行数和可用文字框
- 对高风险文本使用 `--target-pt <参考字号>`；工具会返回 `required_box_px` 与 `box_deficit_px`
- 出现字号偏小而不溢出时，先扩大文本框、减少重复 margin、调整图标/分隔线/列宽占位，再考虑减小字号
- `fits: true` 只说明不越界；最终必须以 1:1 渲染裁图检查字高、宽度、基线和参考层级
- 勾选清单、步骤列表等重复组件不能共用固定文字宽高；先锁定图标槽，再逐行分配剩余宽度，并对中英混排长标签记录 `reference_scale`。同组单行文本若低于参考字高的 `90%`，必须先修文本槽或卡片几何
...
这个 skill 的目标不是生成一张“像 PPT 的图片”，而是把静态视觉页重建成可继续工作的 PowerPoint 系统。

- 先理解结构，再复刻画面：文本、表格、卡片、箭头、图标和复杂视觉各归其位。
- 复杂资产交给生图模型，版式关系交给原生 PPT 对象，保证观感与可编辑性同时成立。
- 每一页都按独立作品处理：逐页拆解、逐页渲染、逐页校准，再合并成完整 deck。
- 把字体、网格、裁切、图层和渲染 QA 沉淀为流程约束，让复刻结果可复用、可追溯、可迭代。
