...
3. **Generate icons and visual assets**
   ...
   - For square `N×N` grids, request a square canvas and `N` equal rows/columns. For rectangular `C×R` grids, request a canvas whose aspect ratio matches `C:R`. Avoid edge-to-edge icons and avoid placing important icons in sheet edge cells when fidelity matters.
   - Each cell must be cut into exactly one semantic asset. Avoid mixing multiple icons into one asset.
   - Do not trust grid math alone. After cutting an asset grid, remove cross-cell fragments and inspect a contact sheet of the final independent PNGs. If any icon contains stray arcs, dots, wedges, clipped strokes, partial marks from adjacent cells, or edge-cell clipping from the generated sheet itself, regenerate that asset as a standalone image before inserting it into PPT.
...
- Icon clipping usually comes from hard crops or transparent padding being lost. Fix by generating true RGBA assets, cutting by grid cell, then repacking content into a larger transparent canvas.
...
- Asset sheets can leak adjacent-cell fragments into button icons. Treat stray bottom arcs, dots, wedges, or partial marks as a real cutting failure even if the standalone asset passes padding checks. Fix with tighter alpha-component cleanup, recutting, or regeneration, then re-render the actual PPT crop.
- Edge cells in a generated icon sheet may be clipped before post-processing even starts. When an edge-cell icon is important, inspect the final PNG itself at thumbnail size. If the icon shape is cut off, do not resize or crop around it; run image generation again for that single isolated asset and replace the grid-derived PNG.
...
- Text wrapping must be checked in rendered PPT, not only by approximate coordinates. Widen boxes or reduce font size when Chinese labels wrap differently in PowerPoint.
